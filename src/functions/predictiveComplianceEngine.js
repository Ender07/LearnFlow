...
export const predictiveComplianceEngine = async () => {
  try {
    const certifications = await base44.db.Certification.list();
    const allProgress = await base44.db.UserProgress.filter({ status: 'certified' });
    ...
    for (const gap of criticalGaps) {
      ...
      const existing = await base44.db.UserProgress.filter({
        process_id: cert.renewal_process_id,
        created_by_id: gap.user_id,
      });
      if (existing.length > 0) continue;

      const newProgress = await base44.db.UserProgress.create({
        process_id: cert.renewal_process_id,
        status: 'not_started',
    ...
    await base44.db.ComplianceReport.create({
    ...
    return {
      data: { gaps, auto_assigned_count: autoAssigned.length, total_at_risk: gaps.length }
    };
  } catch (error) {
